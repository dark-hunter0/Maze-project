...
def maze_creator_csv(file_path):
    ...
    with open(file_path, 'r') as file:
        ...
        for row in reader:
            maze.append([int(float(cell)) for cell in row])  # Convert to float first, then to int

    entry, exit = find_points(maze)
    display_maze(maze, entry, exit)

# Function to find entry and exit points in the maze
def find_points(maze):
    entry = None
    exit = None
    for i in range(len(maze[0])):  # Check top and bottom rows
        if maze[0][i] == 0:
            entry = (0, i)
        if maze[-1][i] == 0:
            exit = (len(maze) - 1, i)
    ...
    if not entry or not exit:
        for i in range(len(maze)):  # Check left and right columns
            if maze[i][0] == 0:
                entry = (i, 0)
            if maze[i][-1] == 0:
                exit = (i, len(maze[0]) - 1)
